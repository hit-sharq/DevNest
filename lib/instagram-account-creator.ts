...
export class InstagramAccountCreator {
  ...
  private async getSMSActivatePhone(): Promise<string | null> {
    try {
      ...
      const response = await fetch(
        "https://sms-activate.org/stubs/handler_api.php?api_key=YOUR_API_KEY&action=getNumber&service=ig&country=0",
      )
  ...
  private generatePassword(): string {
    return PasswordManager.generateSecurePassword(16)
  }
  ...
  private async randomDelay(baseDelay = 3000): Promise<void> {
...
export class AccountCreationManager {
  ...
  async startScheduledCreation(accountsPerDay = 10) {
